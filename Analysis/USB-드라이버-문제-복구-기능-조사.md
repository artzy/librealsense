...
| 상황 | 프로젝트 내 대응 | 예제·함수·도구 | 비고 |
|------|------------------|----------------|------|
| Recovery 모드 (USB에 `D4XX Recovery`로 보임) | ✅ `rs-fw-update -r` | `tools/fw-update/rs-fw-update.cpp` → `update_recovery_device()`; `device.is_in_recovery_mode()`; `common/fw-update-helper.cpp` → `firmware_update_manager::update()` | 가장 직접적인 복구 |
| 장치 hang, 응답은 있음 | ✅ Viewer **Hardware Reset** / API | `device::hardware_reset()` / `dev.hardware_reset()` (C++/Python); `d400_device::hardware_reset()` (`src/ds/d400/d400-device.cpp`); Viewer `common/device-model.cpp`; `tools/dds/dds-config/rs-dds-config.cpp` `--reset`; `wrappers/python/examples/depth_ucal_example.py` | USB 재열거 유도 |
| 스트리밍 중 프레임 멈춤 | △ SDK 내부 `reset_endpoint` | `src/uvc/uvc-streamer.cpp` → `messenger->reset_endpoint()`; Windows `WinUsb_ResetPipe` (`src/winusb/messenger-winusb.cpp`); libusb `clear_halt` (`src/libusb/messenger-libusb.cpp`) | 자동, 사용자 도구 아님 |
| 앱 실행 중 USB 재연결 | △ `device_hub`, Pipeline 재시작 | `examples/sensor-control/` → `device_hub.wait_for_device()`; `context.set_devices_changed_callback()`; `src/pipeline/pipeline.cpp` → `unsafe_stop()` / `unsafe_start()`; `tools/terminal/rs-terminal.cpp` → `wait_for_device()` | **재연결 대기** |
| Linux uvcvideo/권한 문제 | △ patch 스크립트, udev, RSUSB | `scripts/setup_udev_rules.sh`; `scripts/patch-realsense-ubuntu-lts.sh`, `patch-realsense-ubuntu-lts-hwe.sh`; CMake `-DFORCE_RSUSB_BACKEND=ON` (`CMake/lrs_options.cmake`); `scripts/libuvc_installation.sh`; `doc/troubleshooting.md` | 빌드/설치 단계 |
| USB 완전 disconnect (Device Manager에도 없음) | ❌ | `rs-enumerate-devices -l`, `rs-fw-update -l` (인식 여부 확인만); `doc/troubleshooting.md` | 케이블/포트/허브/드라이버 재설치 등 **OS/하드웨어** |
| 드라이버 크래시 후 enumerate 안 됨 | ❌ (런타임) | `rs-enumerate-devices`, `rs-fw-update -l` (상태 확인); `doc/troubleshooting.md` (`dmesg`, `lsusb`, `udevadm monitor`) | 재부팅, 드라이버 재설치, `dmesg` 분석 |
